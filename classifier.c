...
int main(int argc, char *argv[]) {
  ...
  // while ((opt = getopt(argc, argv, "training_data:testing_data:")) != -1){
  //   switch (opt)
  //   {
  //   case "training_data":
  //     /* code */
  ...
  filetrain = argv[1];
  filetest = argv[2];
  Dataset *data = load_dataset(filetrain);
  DTNode *tree = build_dec_tree(data);
  Dataset *vali = load_dataset(filetest);
  for (int i=0; i<vali->num_items;i++){
    Image *image = &(vali->images[i]);
    if (vali->labels[i] == dec_tree_classify(tree,image)){total_correct+=1;}
  }
  free_dec_tree(tree);
  free_dataset(data);
  free_dataset(vali);
